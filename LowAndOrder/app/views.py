...
def adminLog(request):
    ...
    if (uname=="admin" and upass=="admin"):
        return render(request,"adminHome1.html",{"msg":"Welcome Admin"})
    else:
...
def DetecReport(request):
    return render(request,"DetecReport.html")

def CitizReport(request):
    return render(request,"citizReport.html")
...
def findDetecReport(request):
    status=request.POST.get("status")
    qs= Status.objects.filter(status=status)
    print(status,qs)
    return render(request, "detecReport.html",{"data":qs})

def findCitizReport(request):
    status=request.POST.get("status")
    qs= Status.objects.filter(status=status)
    print(status,qs)
    return render(request, "citizReport.html",{"data":qs})
